...
python3 .../tavern_cli.py add-original card.json --name "雾港来信"

# RIGHT — imports card, then attaches to existing world
python3 .../tavern_cli.py add-original card.json
python3 .../tavern_cli.py attach-card "雾港来信" "card_<id>"
```
...
`add-lore` auto-extracts trigger keys from the content. Terms like "组织", "成员", and "商会" can overlap with existing entries. Always run `lore-audit` after `add-lore` and fix the event entry's keys to be narrow and specific.
