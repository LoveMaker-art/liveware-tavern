...
description: Add, test, switch, or repair official and third-party model APIs for Hermes Agent, Tavern, or both without mixing their configuration scopes.
version: 1.0.0
...
Load `${HERMES_SKILL_DIR}/references/provider-protocols.md` when classification
...
```sh
python3 ${HERMES_SKILL_DIR}/scripts/model_api_manager.py inspect --target both
python3 ${HERMES_SKILL_DIR}/scripts/model_api_manager.py store-key --env PROVIDER_API_KEY
python3 ${HERMES_SKILL_DIR}/scripts/model_api_manager.py probe \
  --target both --base https://example.com/v1 --model model-id \
  --key-env PROVIDER_API_KEY
python3 ${HERMES_SKILL_DIR}/scripts/model_api_manager.py apply \
...
For an official native Hermes provider, follow
`${HERMES_SKILL_DIR}/references/hermes-model.md`. For Tavern behavior and its
existing specialist commands, load
`${HERMES_SKILL_DIR}/references/tavern-model.md`.
...
Load `${HERMES_SKILL_DIR}/references/security-and-rollback.md` for failures,
