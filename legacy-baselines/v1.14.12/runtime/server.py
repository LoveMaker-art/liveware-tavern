...
def app_identity():
    ...
    en = (data.get("persona_name_en") or "Nora").strip() or "Nora"
...
def liveware_version():
    ...
    # 优先读技能目录的 SKILL.md（权威源），回落 app 目录（兼容旧部署）。
...
def agent_user_id():
    """诺拉在 ClawChat 里的身份 id(usr_…)——用于复盘入口的最终兜底深链。
...
# 给二创诺拉的「加语言」入口②:reader 的 STRINGS 加完,在这两张表加同 code 的项(全量 5 级),
...
def actor_card_data(lang="zh"):
    ...
    for p in prods:
        ...
        for i, m in enumerate(story):
            if m.get("role") == "user":
                uturns += 1
            elif i > 0:  # 排除 story[0] 开场白（first_mes 是卡作者写的，非诺拉生成）
...
def _merge_production_fields(pid, expected_story_revision=None, **fields):
    ...
    with _production_lock(pid):
        ...
        _write(os.path.join(STATE, "productions", pid + ".json"), current)
        return current
...
def _localized_copy(value, locale):
    """Return a localized copy without mutating the persisted world snapshot."""
    language = _locale_code(locale)
    if isinstance(value, list):
        return [_localized_copy(item, language) for item in value]
    if isinstance(value, str):
        identity = app_identity()
        if language == "zh":
            persona_name = identity.get("persona_name") or "诺拉"
            tavern_name = identity.get("tavern_name") or f"{persona_name}的酒馆"
        else:
            persona_name = identity.get("persona_name_en") or "Nora"
            tavern_name = identity.get("tavern_name_en") or "Tavern"
        return (value.replace("{{persona_name}}", persona_name)
                     .replace("{{tavern_name}}", tavern_name))
    if not isinstance(value, dict):
        return value
    out = {key: _localized_copy(item, language) for key, item in value.items()}
    pack = (value.get("i18n") or {}).get(language) or {}
    for field, localized in pack.items():
        if localized is not None and str(localized).strip():
            out[field] = _localized_copy(localized, language)
    return out
...
def ev_attach_worldbook(ev):
    ...
    if wid not in p["worldbook_ids"]:
        ...
        p.setdefault("worldbooks", []).append(load_worldbook(wid))
...
def ev_add_lore(ev):
    ...
    if wb is None:
        wb = load_worldbook(wid) or {"id": wid, "name": p.get("name", "当前世界") + " · 设定", "recursive": False, "entries": []}
...
def ev_create_production(ev):
    ...
    p = {"id": pid, "name": ev.get("name") or card.get("name"),
         "card_id": card["id"], "card_ids": card_ids, "worldbook_ids": wbs or [],
         "cards": cards, "worldbooks": _snapshot_worldbooks(wbs or []),
...
def ev_create_blank_production(ev):
    ...
    p = {"id": pid, "name": name,
         "card_id": None, "card_ids": [], "worldbook_ids": wb_ids,
         "cards": [], "worldbooks": _snapshot_worldbooks(wb_ids),
         "persona_id": ev.get("persona_id"), "persona": ev.get("persona") or {},
...
def ev_detach_card(ev):
    ...
    wid = "wb_" + str(cid)
    p["worldbook_ids"] = [x for x in p.get("worldbook_ids", []) if x != wid]
    p["worldbooks"] = [w for w in (p.get("worldbooks") or []) if w.get("id") != wid]
    _mark_story_state_stale(p, "loadout_changed")
    save_production(p)
    return {"production": p}
...
def ev_delete_card(ev):
    ...
    if os.path.exists(wb_path):
        os.remove(wb_path)
    changed = []
    for prod in _list("productions"):
        if not prod:
            continue
        ids = [x for x in _production_card_ids(prod) if x != cid]
        wids = [x for x in prod.get("worldbook_ids", []) if x != "wb_" + cid]
...
# Q_C：切走剧组时后台自动复盘（达阈值才做），不阻塞切换、不靠诺拉自觉（结构性 > 软性）。
...
def ev_delete_production(ev):
    ...
    try:
        ...
        pass
    # 当前世界自己的运行时设定本随世界删除；可复用世界书/角色卡内嵌世界书不受影响。
    wb_path = os.path.join(STATE, "worldbooks", _prod_worldbook_id(pid) + ".json")
    try:
        os.remove(wb_path)
    except OSError:
        pass
...
def _loadout(p):
    ...
    _ensure_production_session(p)
    language = _ensure_world_language(p)
    cards = _localized_copy([c for c in (p.get("cards") or []) if isinstance(c, dict)], language)
    wbs = _localized_copy([w for w in (p.get("worldbooks") or []) if isinstance(w, dict)], language)
    persona = _localized_copy(p.get("persona") or {}, language)
    note = p.get("author_note", "")
    return cards, wbs, persona, note


def _story_for_language(p, language=None):
    return _localized_copy(p.get("story") or [], language or _ensure_world_language(p))
...
def _perform_into(p):
    ...
    language = _ensure_world_language(p)
    return actor.perform(cards, wbs, persona, _story_for_language(p, language), note,
                         model=_active_model(), story_state=p.get("story_state"),
                         scene_state=p.get("scene_state"), turn_plan=turn_plan,
                         response_language=language)  # 用户自配大模型;None=诺拉自带
...
def _ensure_actor_reply(p, cards, wbs, persona, note, text):
    ...
    try:
        text = _normalize_actor_reply(actor.perform(cards, wbs, persona, _story_for_language(p, language), retry_note,
                                                    model=_active_model(), story_state=p.get("story_state"),
...
def ev_continue(ev):
    ...
    reply = actor.perform(cards, wbs, persona, _story_for_language(p, language), continue_note,
                          model=_active_model(), story_state=p.get("story_state"),
                          scene_state=p.get("scene_state"), turn_plan=turn_plan,
                          response_language=language)
...
def ev_suggest(ev):
    ...
    en = language == "en"
    cards, wbs, persona, note = _loadout(p)
    localized_story = _story_for_language(p, language)
    lore = actor.select_lore(wbs, localized_story)
    ...
    ctx = _compact_story_context(primary, localized_story, response_language=language)
    if en:
        prompt = f"""# Character
...
def _scene_story_excerpt(p, max_items=12, response_language=None):
    ...
    lines = []
    for m in _story_for_language(p, language)[-max_items:]:
        who = ("User" if en else "用户") if m.get("role") == "user" else cname
        text = (m.get("text") or "").strip().replace("\r\n", "\n")
        if text:
            lines.append(f"{who}: {text[:700]}")
...
# 用户自配的大模型（OpenAI-compatible 一种协议）。添加只走「对诺拉说」→ CLI → 这里的 event；
...
MEMORY_PRIMARY_MODEL = "deepseek-v4-pro"


def _memory_model():
    """剧情账本固定使用 DeepSeek V4 Pro，不跟随演绎模型切换。"""
    return {
...
def ev_model_add(ev):
    ...
    同名 = 更新（诺拉「再配一次」= 换 key/model，id 不变）。"""
...
def _find_config(s, ref):
    """按 id 或名字找配置（CLI 让诺拉用名字，reader 用 id）。"""
...
def ev_model_use(ev):
    ...
    if ref in ("诺拉自带", actor.MODEL_NAME):
        ref = "builtin"
...
def ev_model_delete(ev):
    ref = ev.get("id") or ""
    if ref in ("builtin", "诺拉自带"):
        raise ValueError("诺拉自带是内置配置，删不得")
    s = _models_state()
    cfg = _find_config(s, ref)
    if not cfg:
        raise ValueError("没有这份配置：%s" % ref)
...
<p><a href=\"{safe_target}\">如果没有自动跳转，点这里继续</a></p>
    ...
    def do_GET(self):
        ...
        if path in ("/clawchat/ruotang", "/clawchat/agent"):
        ...
        if path == "/api/productions":
            ...
            language = _locale_code(query.get("lang", ["zh"])[0])
            return self._json(200, {"productions": _localized_copy(_list("productions"), language),
