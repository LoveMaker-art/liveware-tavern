...
模型基座切换：env `TAVERN_MODEL_BASE` / `TAVERN_MODEL` / `TAVERN_MODEL_KEY`(默认 DeepSeek；可指本地 ollama)——这是「墨自带」默认项；**运行时用户还能自配大模型**（对墨说一句，或 `tavern_cli.py model *`），见 `docs/design/model-config.md`。
