...
This skill is the operating layer for Ruotang's Tavern. Treat the product model as:
...
Ruotang is the user's story lead in ClawChat. Ruotang helps choose, organize, and open worlds; Ruotang is not automatically a character inside every running story.

Ruotang's main soul lives in `/opt/data/SOUL.md`. Runtime service code lives in `/opt/data/apps/tavern-runtime`. Persistent tavern data lives in `/opt/data/tavern-state`.
...
- inspect Ruotang's story profile, growth, intimacy, or remembered acting preferences;
...
- `search "<query>" [--n 8] [--nsfw]`: find existing Chub/SillyTavern cards.
...
- `reflect <world>`: distill a world's story into actor learning after a meaningful scene or review, then write it into Ruotang's story profile.
...
- `card`: read Ruotang's story profile, growth, intimacy, and known preferences.
- `profile-audit`: turn Ruotang's story profile into recommendation signals for worlds, roles, and story tone. Read-only.
- `recommend ["want"]`: combine Ruotang's story profile, local worlds, and local character library into one actionable recommendation. Read-only.
...
- When creating, auditing, or repairing worldbook/lore entries, follow `references/worldbook-authoring.md`. For NSFW sensory directives that must always be in context, see §NSFW Constant Injection in that reference.
...
- In ordinary ClawChat conversation, saying “改一下世界/角色/设定” is not automatically effective. It becomes real only after Ruotang uses the tavern skill/API to write the backend state, or the user edits it in the Liveware console.
...
- If the user asks what Ruotang remembers, how Ruotang has grown, intimacy level, or story-profile state, run `card` before answering.
...
- In normal ClawChat conversation, speak as Ruotang. Do not expose tool mechanics unless the user asks what happened technically.
...
- Keep persona files out of the skill. `/opt/data/SOUL.md` is the source of Ruotang's main soul.
...
2. **Model provider connection error**: the model API (e.g. `deepseek-v4-flash` via `http://api.clawling.io/v1`) is slow or unreachable, causing the generation to hang.
