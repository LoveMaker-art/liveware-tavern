...
def _event(ev):
    ...
    （别走 _http 的 HTTPError 分支打「HTTP 500 + 原始 JSON」，若棠要把错误读给用户）。"""
...
def cmd_recall(a):
    # 读某个世界在控制台里实际演了什么（若棠在 ClawChat 里看不到 production.story，
...
def _active_name():
    ...
    return next((c["name"] for c in configs if c["id"] == active), "若棠自带")
...
def cmd_model_list(a):
    ...
    for c in configs:
        ...
        if c.get("builtin"):
            ...
            print(f"  {mark} 若棠自带 — {c.get('model', '')}（{key}）")
        else:
...
def cmd_profile_audit(a):
    """Read Ruotang's story profile and turn it into recommendation signals."""
    d = _actor_profile()
    c, it = d.get("career", {}), d.get("intimacy", {})
...
def main():
    ap = argparse.ArgumentParser(prog="tavern_cli", description="若棠酒馆工具")
    ...
    s = sub.add_parser("card", help="读若棠的故事档案（生涯/亲密度/对用户的了解/成长）——自我觉察")
    ...
    s = sub.add_parser("recall", help="读某世界在酒馆里走过什么（若棠读酒馆对话的唯一入口）")
    ...
    m = ms.add_parser("use", help="切换在用配置(名字或 id;「若棠自带」= 切回默认)")
    m.add_argument("which", help="配置名 / id / 若棠自带")
    m.set_defaults(fn=cmd_model_use)
    m = ms.add_parser("rm", help="删一份配置(删的是在用的会自动回落若棠自带)")
    m.add_argument("which", help="配置名 / id")
    m.set_defaults(fn=cmd_model_rm)
    m = ms.add_parser("test", help="实测某配置通不通(不给参数=测若棠自带)")
    m.add_argument("which", nargs="?", help="配置名 / id;缺省=若棠自带")
