# Tavern Ruotangdel Configuration
...
```sh
curl -s http://api.clawling.io/v1/models -H "Authorization: Bearer <key>"
```
...
- Ruotangdel configs are stored under `/opt/data/tavern-state` and should remain server-side only.
- Default/builtin model should not be deleted. Use `model use 若棠自带` to return to it.
