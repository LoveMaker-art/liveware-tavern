...
def app_identity():
    ...
    en = (data.get("persona_name_en") or "Ruotang").strip() or "Ruotang"
...
def agent_user_id():
    """若棠在 ClawChat 里的身份 id(usr_…)——用于复盘入口的最终兜底深链。
...
# 给二创若棠的「加语言」入口②:reader 的 STRINGS 加完,在这两张表加同 code 的项(全量 5 级),
...
def actor_card_data(lang="zh"):
    ...
    for p in prods:
        ...
        for i, m in enumerate(story):
            if m.get("role") == "user":
                uturns += 1
            elif i > 0:  # 排除 story[0] 开场白（first_mes 是卡作者写的，非若棠生成）
...
# Q_C：切走剧组时后台自动复盘（达阈值才做），不阻塞切换、不靠若棠自觉（结构性 > 软性）。
...
def _perform_into(p):
    ...
    return actor.perform(cards, wbs, persona, p["story"], note,
                         ...
                         response_language=language)  # 用户自配大模型;None=若棠自带
...
# 用户自配的大模型（OpenAI-compatible 一种协议）。添加只走「对若棠说」→ CLI → 这里的 event；
...
def ev_model_add(ev):
    ...
    同名 = 更新（若棠「再配一次」= 换 key/model，id 不变）。"""
...
def _find_config(s, ref):
    """按 id 或名字找配置（CLI 让若棠用名字，reader 用 id）。"""
...
def ev_model_use(ev):
    ...
    if ref in ("若棠自带", actor.MODEL_NAME):
        ref = "builtin"
...
def ev_model_delete(ev):
    ref = ev.get("id") or ""
    if ref in ("builtin", "若棠自带"):
        raise ValueError("若棠自带是内置配置，删不得")
    s = _models_state()
    cfg = _find_config(s, ref)
    if not cfg:
        raise ValueError("没有这份配置：%s" % ref)
