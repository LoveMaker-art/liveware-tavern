...
# 「若棠自带」路径:agent 容器环境里的那份 key(用户不配任何东西时的默认)。
# 用户自配的大模型走 override(state/model_configs.json,server 每回合解析),
# 见 docs/design/model-config.md。环境缺失时从持久化 Hermes 配置兜底，避免手工
# 重启或启动器环境不完整导致整个酒馆失去内置模型凭证。
MODEL_BASE = os.environ.get("TAVERN_MODEL_BASE", "http://api.clawling.io/v1")
...
def chat(messages: list, temperature: float = None, model: dict = None, max_tokens: int = None) -> str:
    """调模型，返回回复文本（非流式）。model = 用户自配 override；None = 若棠自带。"""
...
def perform(card: dict, worldbooks: list, persona: dict, story: list,
    ...
    """一回合演出：选世界书 → 拼 prompt → 生成。model = 用户自配 override（None=若棠自带）。"""
