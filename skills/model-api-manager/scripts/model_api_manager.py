...
HERMES_ENV = Path(os.environ.get(
    "HERMES_ENV_PATH", HERMES_HOME / ".env")).expanduser().resolve()
TAVERN_CONSOLE = os.environ.get("TAVERN_CONSOLE", "http://127.0.0.1:8799").rstrip("/")
TAVERN_MODELS = Path(os.environ.get(
    "TAVERN_MODELS_PATH", DATA_ROOT / "tavern-state/model_configs.json")).expanduser().resolve()
TAVERN_CLI = Path(os.environ.get(
    "TAVERN_CLI",
    HERMES_HOME / "skills/creative/tavern/scripts/tavern_cli.py",
)).expanduser().resolve()
