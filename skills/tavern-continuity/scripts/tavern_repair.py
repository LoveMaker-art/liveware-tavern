...
def state_dir():
    ...
    root = os.environ.get("TAVERN_DATA_ROOT", home)
    return os.path.abspath(os.path.expanduser(
        os.environ.get("TAVERN_STATE_DIR", os.path.join(root, "tavern-state"))
    ))
...
def runtime_actor_chat(messages, temperature=0.1):
    ...
    root = os.environ.get("TAVERN_DATA_ROOT", home)
    runtime = os.path.abspath(os.path.expanduser(
        os.environ.get("TAVERN_APP_DIR", os.path.join(root, "apps", "tavern-runtime"))
    ))
