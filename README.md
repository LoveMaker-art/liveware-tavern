...
- `creative-skills/` - The lightweight Tavern router plus six specialist Hermes skills.
...
The Agent can then check and install a verified stable release after explicit user confirmation. Any instance whose updater is older than `v1.21.0` must enter this transition through the one-command Bootstrap so the updater is upgraded before it reviews the expanded runtime-module allowlist and exact-directory skill manifest.
