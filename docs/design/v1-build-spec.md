...
> **两个「版本」别混（2026-06-30）**：① **活件版本** = 酒馆这个 **app** 自己的发版号（agent 改了功能/皮肤再部署 = 一次发版），取 `SKILL.md` `version`，reader 右栏底显「活件 · 酒馆 v1.0.0」。② **演员成长版本** = 演员墨技艺层（`actor_self.md`）随演出/复盘**积累**的程度，是另一个维度（上面的 `actor_self.meta.json`，v1.1+）。reader 把这两件事**分开呈现**：演员成长的完整呈现在**演员卡 surface**（panel「演员 · 墨」只留两个入口：找墨复盘 + 墨的演员卡，2026-07-02 收敛，原摘要/手记撤）；活件版本是应用层的安静 footer。
...
| `actor_grow {change, reason}` | learn：把变化**合并进「我对你的了解」段**（`actor.merge_knows`：去重/精化/有界≤12，**非尾部堆**）+ 在「成长记」记一笔生涯年表审计（带人话理由）。**Q_B 已实现 2026-07-01**（本就是"改写相应段"原意，收敛旧 code↔doc 冲突；meta 快照仍 v1.1+） | actor_self |
| `reflect {production_id}` | 复盘整场戏 → 蒸馏偏好 → **合并进「我对你的了解」** + 记生涯年表（「越演越懂你」的**结构化触发**，不靠 agent 临场）。**Q_C：另在切走剧组时服务端后台自动触发**（`_maybe_auto_reflect`，达轮数阈值才做，不阻塞切换）—— 从"显式 only"提前 | learned |
...
读路由（GET，仿 digest read 路由）：`/api/cards`、`/api/worldbooks`、`/api/productions`、`/api/actor`（返回 `{actor_self, version}`——`actor_self` = 演员技艺层全文，`version` = **活件版本** = 酒馆 app 自己的发版号，取 `SKILL.md` frontmatter `version`，见 §3 注）、**`/api/actor_card`**（演员卡聚合：生涯数值/亲密度/口味/年表，纯聚合读现有 state，见 `actor-card.md`）。

页面路由：`/`（console 控制台）、**`/actor`**（**演员卡** surface，第二个活件卡入口，见 `actor-card.md` + `liveware-frontend.md §5`；同一 `server.py`、共享 `console.css`，`ac*` 前缀）。
...
- **来源 = Chub.ai**（角色卡事实标准库，6 万+，公开 API 免鉴权）：搜 `https://api.chub.ai/search?search=…`；下载真卡 `https://avatars.charhub.io/avatars/<fullPath>/chara_card_v2.png`（带 chara chunk，喂 `card_import`）。**⚠️ 可达性依赖网络/代理**——容器实测是透明借宿主 Clash 出去的，换一台没配代理/墙内的机器就断。**降级 = 随仓 8 张 starter 真卡兜底**（`search`/`add` 探测网络不可达 → 列内置卡、绝不回退手搓 PNG；决策 + 卡集 + 复制即用边界见 `find-cards.md`，2026-07-01）。
- **CLI `tools/tavern_cli.py`**（纯 stdlib，POST 本地控制台 `/api/event` + GET 读路由）：`search "<q>"` / `add <fullPath\|Chub链接>`（拉真卡→`import_card`→`create_production`；`_parse_full_path` 吃用户直贴的链接）/ **`starter [<序号\|名字>]`**（随仓内置 starter 真卡：不给参数=列表、给=导入建剧组；Chub 不可达兜底 + 墨写原创卡的结构样板）/ `add-original <json>`（`import_card_json`，原创卡）/ `add-worldbook <json> [--production]`（`import_worldbook`+`attach_worldbook`）/ `list` / `card`（读 `/api/actor_card`——墨的**自我觉察**：生涯/亲密度/口味/成长，引用成长 + 里程碑丢演员卡活件，基础线 2）/ `recall <剧组> [--last N]`（读该剧组 `story`——墨读酒馆对戏的入口）/ `learn "<…>" [--reason]`（`actor_grow`，**合并进「我对你的了解」+ 记年表**）/ `reflect <剧组>`（复盘整场→蒸馏偏好→合并进技艺层 + 记年表；切走剧组时服务端 auto 触发）。
- **记忆桥接（持久搭子的关键，2026-06-29）**：控制台对戏存 `production.story`、走同源 server，**不经 gateway**，所以 ClawChat 里的墨默认看不到（实测墨会答「我看不到酒馆里聊了什么」）。`recall` 让墨按需读某场戏（故事隔离不变——只是墨的元认知能读，不往别的剧组生成里串）；`learn`→`actor_grow` 让墨把「对你的了解」沉淀进跨剧组共享的 `actor_self`（注入每场生成）。两者合起来 = 「越演越懂你」。注：另有 Hermes 自带 user-profile 记忆层，RP 演法/口味走我们的 `actor_self`（它喂得到控制台），通用事实可留给 Hermes。**结构性 > 软性（2026-06-29 教训）**：光改 SOUL 扭 agent 行为不可靠（墨曾无视「用 add-original」继续手搓 PNG、复盘也不自发 `learn`）——所以①「越演越懂你」用 `reflect`（**服务端模型蒸馏**，不靠 agent 临场）②原创卡不手搓 PNG，靠**删掉墨自创的竞争产物**——`sillytavern-character-cards` 技能（标了废弃却仍 enabled、诱它走 PNG）+ `skills/creative/tavern/references/browser-injection-cjk-success.md`（墨自写的「浏览器手搓 PNG 注入」操作手册，躺在技能目录里反噬「绝不手搓」；2026-07-01 删）+ `starter` 卡消灭「没卡才手搓」的动机 + `card_import` 解析层兜底乱码。结构性机制比改 prompt 靠谱。
...
- **技艺层学习**：合并写入（`merge_knows`）+ 显式 `learn`/`reflect` + **服务端 auto-reflect（切走剧组触发，Q_C，2026-07-01 提前）**；「我对你的了解」注入生成、「成长记」只上演员卡年表**不进 prompt**（Q_B 注入瘦身）。从对话文本**自动识别**元指令（"回复短点"→自动 learn）仍 = v1.1+。
