...
- ✅ **安装自动化（2026-07-01 落地）**：一键 `tools/install.sh <容器>` 合并两处放置（技能注册 `skills/creative/tavern/SKILL.md` + 运行时 `tavern/`）+ 竞争产物卫生 + creds 自检，再调 `provision.sh`（建/复用两个 app、写 `state/apps.json`、register）+ `bringup.sh`（起 Loop B）。`bringup.sh` 已泛化为读 `state/apps.json`（不再写死作者的 app ID），职责收窄到「重启恢复」；建 app / register 挪进 provision。完整流程、幂等/配额规则、C 侧前置见 **`install.md`**。
