---
name: tavern-updater
description: Review, merge, install, and roll back verified Tavern releases from LoveMaker-art/liveware-tavern. Use when the user asks to check, compare, audit, hot-update, or roll back Tavern backend, official frontend code, the seven creative Tavern skills, or the release-managed AGENTS.md on a Hermes/ClawChat instance. This skill updates only explicit allowlists and never updates identity, persona, assets, starter content, fixtures, or persistent data.
---
...
- Update only allowlisted backend files, the seven official `runtime/web` code files, the seven creative skills, the complete release-managed `/opt/data/AGENTS.md`, and this updater; update the updater last.
...
- Apply and roll back individual runtime and skill files atomically. Replace `/opt/data/AGENTS.md` as one complete file after backing up its prior contents. Never replace the complete runtime or creative-skill root.
